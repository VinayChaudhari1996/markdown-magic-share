...
import { ZoomIn, ZoomOut, Edit, Eye } from "lucide-react";
    ...
    >
      ...
      <div className="relative max-w-[1200px] mx-auto w-full">
        <Button
          ...
          className="absolute -left-14 top-1/2 -translate-y-1/2 rounded-full bg-white/80 backdrop-blur-xl border-0 shadow-sm hover:bg-white/90 z-10"
          aria-label={isEditorVisible ? "Hide editor" : "Show editor"}
        >
          {isEditorVisible ? <Eye className="h-4 w-4" /> : <Edit className="h-4 w-4" />}
        </Button>

        <ResizablePanelGroup
          direction="horizontal"
          className="flex-1 rounded-2xl border bg-white/80 backdrop-blur-xl shadow-sm overflow-hidden"
        >
          <AnimatePresence initial={false}>
            {isEditorVisible && (
              <motion.div
                ...
                className="h-full"
                style={{ minWidth: isEditorVisible ? "30%" : 0 }}
              >
